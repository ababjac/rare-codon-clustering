...
def get_bin_counts(list, bins):
    if len(list) == 0:
        return np.histogram(list, bins)[0]

    hist = np.histogram(list, bins)
    l = np.divide(hist[0].tolist(), len(list))
    return l
...
df = pd.read_csv('../Data/Ecoli/shuf_seqs_ecoli.csv')

aa_freq_ecoli = get_aa_frequencies_list(codon_freq_ecoli, amino_acid_table, codon_table)

minMax_by_row = []
#for seq in df['mRNA']:
for seq in df['shuf_seqs']:
    if (len(seq) % 3) == 0:
        minMax_by_row.append(calc_minMax_percent(seq, aa_freq_ecoli, codon_freq_ecoli, codon_table, WINDOW_SIZE))
    else:
...
histograms = []
for MM_list in X_new:
    histograms.append(get_bin_counts(MM_list, BINS))

#creating upper triangular matrix using hellinger distance
HD = [[0]*len(histograms) for i in range(len(histograms))]
for i in range(len(histograms)):
    for j in range(i+1, len(histograms)):
        # if not histograms[i] or not histograms[j]:
        #     HD[i][j] = float('NaN')
        #     HD[j][i] = float('NaN')
        # else:
        h = hellinger(histograms[i], histograms[j])
        HD[i][j] = h
        HD[j][i] = h

out = pd.DataFrame(HD)
out.to_csv('../Files/MM/HD_ecoli_shuf.csv')
